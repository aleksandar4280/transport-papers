...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: oldPaper } = await supabaseAdmin.from('papers').select('*').eq('id', params.id).single()
  if (!oldPaper) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  let storage_path: string = oldPaper.storage_path
  let content_text: string | null = oldPaper.content_text
  ...
  const newRow = {
    ...
    content_text, storage_path,
  }

  const { error: updErr } = await supabaseAdmin.from('papers').update(newRow).eq('id', params.id)
  ...
  if (authorsCsv) {
    ...
    for (const name of names) {
      const { data: a } = await supabaseAdmin
        ...
        .select().single()
      if (a) await supabaseAdmin.from('paper_authors').insert({ paper_id: params.id, author_id: a.id })
    }
  }
